...
train_data$log_Age <- log(train_data$Age)
test_data$log_Age <- log(test_data$Age)

#SVR
#SVR WITHOUT SCALING
# Fit Support Vector Regression model
svr_model=svm(log_Age ~ ., data = train_data, 
...
train_mse_svr <- mean((train_predictions - train_data$log_Age)^2)

# Make predictions on the test set
test_predictions <- predict(svr_model, test_data)

# Calculate testing MSE for SVR
test_mse_svr <- mean((test_predictions - test_data$log_Age)^2)
...
train_mse_xgb <- mean((train_predictions_xgb - train_data$log_Age)^2)

# Make predictions on the test set
test_predictions_xgb <- predict(xgb_model, as.matrix(Xtest_XG))

# Calculate testing MSE for XGBoost
test_mse_xgb <- mean((test_predictions_xgb - test_data$log_Age)^2)
...
Ytrain <- log(train_data$Age)
Ytest <- log(test_data$Age)
