...
#We do not remove outliers from the testing set
cleaned_test_data = test_data

# SVR
# SVR WITHOUT SCALING
# Fit Support Vector Regression model
svr_model <- svm(Age ~ ., data = cleaned_train_data, 
...
train_mse_svr <- mean((train_predictions - cleaned_train_data$Age)^2)

# Make predictions on the test set
test_predictions <- predict(svr_model, cleaned_test_data)

# Calculate testing MSE for SVR
test_mse_svr <- mean((test_predictions - cleaned_test_data$Age)^2)
...
train_mse_xgb <- mean((train_predictions_xgb - cleaned_train_data$Age)^2)

# Make predictions on the test set
test_predictions_xgb <- predict(xgb_model, as.matrix(Xtest_XG))

# Calculate testing MSE for XGBoost
test_mse_xgb <- mean((test_predictions_xgb - cleaned_test_data$Age)^2)
...
Ytrain <- cleaned_train_data[, 9, drop = FALSE]
Ytest <- cleaned_test_data[, 9, drop = FALSE]
